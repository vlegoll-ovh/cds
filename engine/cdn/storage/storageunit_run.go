...
type withNewReader interface {
	NewReader(index.Item) (io.ReadCloser, error)
	Read(index.Item, io.Reader, io.Writer) error
	Name() string
}
...
func (x *RunningStorageUnits) runItem(ctx context.Context, tx gorpmapper.SqlExecutorWithTx, dest StorageUnit, id string) error {
	...
	reader, err := source.NewReader(*item)
	if err != nil {
	...
	writer, err := dest.NewWriter(*item)
	if err != nil {
	...
	go func() {
		defer pw.Close()
		if err := source.Read(*item, reader, pw); err != nil {
			chanError <- err
		}
		close(chanError)
	}()

	if err := dest.Write(*item, pr, writer); err != nil {
		return err
	}
...
func (x *RunningStorageUnits) NewItemUnit(ctx context.Context, m *gorpmapper.Mapper, tx gorp.SqlExecutor, su StorageUnit, i *index.Item) (*ItemUnit, error) {
	loc, err := su.NewLocator(i.Hash)
	if err != nil {
